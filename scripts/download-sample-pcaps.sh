...
echo "[WLAN 802.11]"
download_gz "wpa-eap-tls.pcap.gz" "module-24-wlan-raw.pcap"

# Generate management frames and merge with real capture
RAW="${PCAP_DIR}/module-24-wlan-raw.pcap"
FINAL="${PCAP_DIR}/module-24-wlan.pcap"

if [ -f "$RAW" ]; then
    if [ ! -f "$FINAL" ] || [ "$RAW" -nt "$FINAL" ]; then
        MGMT_PY="${SCRIPT_DIR}/_gen_wlan_mgmt.py"
        MGMT_PCAP="${PCAP_DIR}/module-24-wlan-mgmt.pcap"
...
PYEOF
        ...
        if [ -f "$MGMT_PCAP" ]; then
            ...
            CLEAN="${PCAP_DIR}/module-24-wlan-mgmt-clean.pcap"
            ...
            TMP="${FINAL}.tmp"
            # -F pcap: pcapng yerine klasik pcap yaz — pcapng Section
            # Header'a mergecap patika/OS metadata'si gomulur (OPSEC)
            mergecap -F pcap -w "$TMP" "$MGMT_PCAP" "$RAW" 2>/dev/null
            mv "$TMP" "$FINAL"
            rm -f "$MGMT_PCAP" "$RAW"
...
ls -lh "${PCAP_DIR}/module-24-wlan.pcap" 2>/dev/null || echo "  (wlan pcap indirilemedi)"
