...
# ── Step 1: SSL Certificate (mini-CA: Root CA + server + OCSP) ──
echo ""
echo -e "${YELLOW}[1/7]${NC} ${BOLD}SSL sertifika kontrolü (mini-CA)...${NC}"
mkdir -p shared/certs shared/pcaps
if [ ! -f shared/certs/fullchain.crt ]; then
    ./shared/ca/generate-ca.sh
    log_ok "Mini-CA ve sertifika zinciri oluşturuldu"
else
